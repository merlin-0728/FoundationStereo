#!/usr/bin/env python3
"""Generate FoundationStereo depth for fixed lowres view pairs.
...
saved under:
  fs_rectified/lowres/<left_view>/
  fs_depth/lowres/<left_view>/
"""
...
FIXED_PAIRS = list(zip(LOWRES_VIEWS[:-1], LOWRES_VIEWS[1:]))
...
def _save_outputs(
    ...
    rect_dir = seq_dir / "fs_rectified" / "lowres" / left_view
    depth_dir = seq_dir / "fs_depth" / "lowres" / left_view
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="FoundationStereo depth with fixed lowres pairs")
...
def main() -> None:
    ...
    for sequence in sequences:
        ...
        for left_view, right_view in FIXED_PAIRS:
            ...
            for frame_id in frame_ids:
                depth_path = seq_dir / "fs_depth" / "lowres" / left_view / f"{frame_id}_depth.npy"
    ...
    summary_path = args.summary_path or (args.data_root / "fixed_pairs_lowres_summary.json")
